...
#' @title Helper: convert NA to zero
...
#' @title RSEM: calculate gene expressions
...
#' @title RSEM: two-sample comparison for DEG identification.
#' @param d.path path to the folder containing "gene.count.matrix" files
...
#' @title RSEM: data export after abundance estimated.
...
RSEM_pipeline <- function(data_dir, treatments, controls,
                          ...
                          DEG.fc=1, annotations=NULL, f.pattern=NULL) {
    ...
    for(i in 1:length(treatments)) {
        ...
        ## expressed gene_id
        snames <- c(cx, tx)
        egids <- apply(exprs %>% select(all_of({{snames}})),
                       1, FUN=function(x) any(x > 1))
        egids <- filter(exprs, egids) %>% pull(gene_id)
        ##
...
#'
#' Get expression data
#' @title RSEM: pull/extract after \code{\link{RSEM_pipeline}}
...
#' @author ZG Zhao
#' @export
RSEM_read_DEGs <- function(files, g.union=FALSE) {
    n <- length(files)
    ans <- NULL
